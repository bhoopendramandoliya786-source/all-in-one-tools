...
function GeneratorTool({ tool }: { tool: Tool }) {
  ...
  const generate = () => {
    ...
    if (tool.slug === 'password-generator') setResult(makeRandomPassword(Math.min(64, Math.max(6, Number(length) || 18)), symbols));
      ...
      if (!value.trim() || !length.trim() || !Number.isFinite(minimum) || !Number.isFinite(maximum) || maximum < minimum) { setError('Enter a valid minimum and maximum.'); return; }
      ...
      if (!value.trim() || !Number.isFinite(numeric)) { setError('Enter a number to convert.'); return; }
    ...
    try {
      ...
      if (tool.slug === 'text-encrypt-decrypt') {
        ...
        if (input.startsWith('ait1:')) {
          ...
          setResult(`ait1:${encode(salt)}:${encode(iv)}:${encode(cipher)}`);
        }
        setBusy(false);
        return;
      }
      let output = input;
      if (tool.slug === 'json-formatter' || tool.slug === 'json-minifier') output = formatCode(input, tool.slug);
      else if (tool.slug === 'base64-encoder-decoder') output = input.startsWith('encoded:') ? atob(input.slice(8)) : `encoded:${btoa(unescape(encodeURIComponent(input)))}`;
      else if (tool.slug === 'url-encoder-decoder') output = decodeURIComponent(input) === input ? encodeURIComponent(input) : decodeURIComponent(input);
      else output = formatCode(input, tool.slug);
      setResult(output);
    } catch { setBusy(false); setError('That input could not be understood. Check the syntax or passphrase and try again.'); setResult(''); }
  };
  return <div className="grid gap-5 lg:grid-cols-[1fr_.85fr]"><div>{tool.slug === 'text-encrypt-decrypt' && <div className="mb-4"><Field label="Passphrase" value={passphrase} onChange={setPassphrase} type="password" placeholder="Required for both directions" /></div>}<textarea data-testid="input-code-content" value={input} onChange={(e) => setInput(e.target.value)} className="min-h-[280px] w-full rounded-2xl border border-input bg-background p-4 font-mono-app text-sm leading-6 outline-none focus:border-primary" placeholder={tool.slug === 'text-encrypt-decrypt' ? 'Paste plaintext or an ait1: value to decrypt...' : `Paste ${tool.name.toLowerCase()} input here...`} /></div><div><div className="flex flex-wrap gap-2"><button data-testid="button-run-transform" onClick={run} disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground disabled:opacity-60"><Zap size={16} /> {busy ? 'Working...' : 'Transform'}</button><button data-testid="button-reset-code" onClick={() => { setInput(''); setPassphrase(''); setResult(''); setError(''); }} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-sm font-bold text-muted-foreground hover:text-foreground"><RefreshCcw size={15} /> Reset</button></div>{error && <div data-testid="status-tool-error" className="mt-4 rounded-xl border border-destructive/25 bg-destructive/10 p-3 text-sm text-destructive">{error}</div>}{result && <ResultBox result={result} onCopy={() => navigator.clipboard?.writeText(result)} onDownload={() => downloadText(`${tool.slug}.txt`, result)} />}</div></div>;
}
...
function ImageTool({ tool }: { tool: Tool }) {
  ...
  const [width, setWidth] = useState('800'); const [height, setHeight] = useState('600'); const [quality, setQuality] = useState('0.8'); const [brightness, setBrightness] = useState('0'); const [contrast, setContrast] = useState('0'); const [watermark, setWatermark] = useState('All in One Tools');
  const process = () => {
    if (!file) { setError('Choose an image first.'); return; }
    setBusy(true); setError('');
    const img = new Image(); const sourceUrl = URL.createObjectURL(file);
    img.onload = () => {
      const isCrop = tool.slug === 'image-crop'; const isResize = tool.slug === 'image-resize';
      const outputWidth = isCrop || isResize ? Math.max(1, Math.min(4000, Number(width) || img.width)) : (tool.slug === 'image-rotate' ? img.height : img.width);
      const outputHeight = isCrop || isResize ? Math.max(1, Math.min(4000, Number(height) || img.height)) : (tool.slug === 'image-rotate' ? img.width : img.height);
      const canvas = document.createElement('canvas'); canvas.width = outputWidth; canvas.height = outputHeight;
      const ctx = canvas.getContext('2d');
      if (!ctx) { URL.revokeObjectURL(sourceUrl); setError('Your browser could not create an image canvas.'); setBusy(false); return; }
      const filters = [
        tool.slug === 'image-grayscale' ? 'grayscale(1)' : '',
        tool.slug === 'image-blur' ? 'blur(5px)' : '',
        tool.slug === 'image-brightness' ? `brightness(${100 + Number(brightness)}%)` : '',
        tool.slug === 'image-contrast' ? `contrast(${100 + Number(contrast)}%)` : '',
      ].filter(Boolean);
      ctx.filter = filters.join(' ') || 'none';
      ctx.save();
      if (tool.slug === 'image-rotate') { ctx.translate(outputWidth / 2, outputHeight / 2); ctx.rotate(Math.PI / 2); ctx.drawImage(img, -img.width / 2, -img.height / 2); }
      else if (tool.slug === 'image-flip') { ctx.translate(outputWidth, 0); ctx.scale(-1, 1); ctx.drawImage(img, 0, 0, outputWidth, outputHeight); }
      else ctx.drawImage(img, 0, 0, outputWidth, outputHeight);
      ctx.restore();
      if (tool.slug === 'image-sharpen') {
        const pixels = ctx.getImageData(0, 0, outputWidth, outputHeight); const copy = new Uint8ClampedArray(pixels.data); const index = (x: number, y: number, channel: number) => (y * outputWidth + x) * 4 + channel;
        for (let y = 1; y < outputHeight - 1; y += 1) for (let x = 1; x < outputWidth - 1; x += 1) for (let channel = 0; channel < 3; channel += 1) pixels.data[index(x, y, channel)] = Math.max(0, Math.min(255, 5 * copy[index(x, y, channel)] - copy[index(x - 1, y, channel)] - copy[index(x + 1, y, channel)] - copy[index(x, y - 1, channel)] - copy[index(x, y + 1, channel)]));
        ctx.putImageData(pixels, 0, 0);
      }
      if (tool.slug === 'image-watermark') { ctx.filter = 'none'; ctx.fillStyle = 'rgba(16, 38, 31, .7)'; ctx.font = `${Math.max(16, Math.round(outputWidth / 18))}px sans-serif`; ctx.fillText(watermark || 'All in One Tools', 18, outputHeight - 20); }
      const outputType = tool.slug === 'png-to-jpg' ? 'image/jpeg' : tool.slug === 'webp-converter' ? 'image/webp' : 'image/png';
      setResultUrl(canvas.toDataURL(outputType, Math.max(0.1, Math.min(1, Number(quality) || 0.8)))); URL.revokeObjectURL(sourceUrl); setBusy(false);
    };
    img.onerror = () => { URL.revokeObjectURL(sourceUrl); setError('This image could not be read in the browser.'); setBusy(false); };
    img.src = sourceUrl;
  };
  const reset = () => { setFile(null); setResultUrl(''); setError(''); setWidth('800'); setHeight('600'); setQuality('0.8'); setBrightness('0'); setContrast('0'); };
  return <div className="max-w-2xl"><label data-testid="dropzone-image" className="flex min-h-[190px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-card p-6 text-center transition hover:border-primary/60 hover:bg-primary/[.03]"><input key={file?.name || 'empty'} data-testid="input-image-file" type="file" accept="image/*" className="sr-only" onChange={(e) => setFile(e.target.files?.[0] || null)} /><span className="grid h-12 w-12 place-items-center rounded-2xl bg-secondary text-primary"><Upload size={21} /></span><span className="mt-3 text-sm font-bold">{file ? file.name : 'Choose an image'}</span><span className="mt-1 text-xs text-muted-foreground">PNG, JPG, WebP · stays on this device</span></label>{(tool.slug === 'image-resize' || tool.slug === 'image-crop') && <div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Width" value={width} onChange={setWidth} type="number" min="1" /><Field label="Height" value={height} onChange={setHeight} type="number" min="1" /></div>}{['image-compress','jpg-to-png','png-to-jpg','webp-converter'].includes(tool.slug) && <div className="mt-4"><Field label="Quality" value={quality} onChange={setQuality} type="number" min="0.1" step="0.1" /></div>}{tool.slug === 'image-brightness' && <div className="mt-4"><Field label="Brightness" value={brightness} onChange={setBrightness} type="number" min="-100" step="10" /></div>}{tool.slug === 'image-contrast' && <div className="mt-4"><Field label="Contrast" value={contrast} onChange={setContrast} type="number" min="-100" step="10" /></div>}{tool.slug === 'image-watermark' && <div className="mt-4"><Field label="Watermark text" value={watermark} onChange={setWatermark} /></div>}{error && <p data-testid="status-image-error" className="mt-3 text-sm text-destructive">{error}</p>}<div className="mt-5 flex flex-wrap items-center gap-3"><button data-testid="button-process-image" onClick={process} disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground disabled:opacity-60">{busy ? 'Working...' : 'Process image'} <ArrowRight size={16} /></button><button data-testid="button-reset-image" onClick={reset} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-sm font-bold text-muted-foreground hover:text-foreground"><RefreshCcw size={15} /> Reset</button>{resultUrl && <a data-testid="link-download-image" download={`${tool.slug}.${tool.slug === 'png-to-jpg' ? 'jpg' : tool.slug === 'webp-converter' ? 'webp' : 'png'}`} href={resultUrl} className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-3 text-sm font-bold hover:border-primary"><Download size={16} /> Download</a>}</div>{resultUrl && <img data-testid="img-processed-result" src={resultUrl} alt="Processed result" className="mt-6 max-h-72 rounded-xl border border-border object-contain" />}</div>;
}
