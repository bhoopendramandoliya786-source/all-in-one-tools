...
import { Download, Upload, RefreshCcw, Lock, Unlock, FileText, Check, Eye, EyeOff, Sliders, Image as ImageIcon, Sparkles, AlertCircle } from 'lucide-react';
...
export default function AdvancedPdfTool({ tool }: { tool: Tool }) {
  ...
  const [startNumberAt, setStartNumberAt] = useState('1');
  ...
  const [hasNupBorder, setHasNupBorder] = useState(true);
  ...
  const [imgDpi, setImgDpi] = useState<'150' | '300'>('300');

  // Compress Settings
  const [targetKb, setTargetKb] = useState('100');
  const [compressedSizeKb, setCompressedSizeKb] = useState<number | null>(null);

  // Invoice & Receipt Cleaner Settings
  const [cleanerMode, setCleanerMode] = useState<'shadow' | 'darken' | 'bw'>('shadow');

  // Duplex Sorter Settings
  const [duplexPattern, setDuplexPattern] = useState<'reverseBacks' | 'normalBacks'>('reverseBacks');
  ...
  const [pdfInfo, setPdfInfo] = useState<{ pages: number; title: string; author: string; size: string; a4: string; dpiGrade: string } | null>(null);
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles(selected);
    setOriginalSizeKb(Math.round(selected[0].size / 1024));
  ...
  const getActionLabel = () => {
    switch (tool.slug) {
      case 'pdf-compress': return 'Compress PDF Now';
      case 'pdf-merge': return 'Merge PDFs in Order';
      case 'pdf-split': return 'Split Selected Pages';
      case 'pdf-rotate': return 'Rotate & Save PDF';
      case 'pdf-delete-pages': return 'Remove Selected Pages';
      case 'pdf-extract-pages': return 'Extract to New PDF';
      case 'pdf-watermark': return 'Stamp Watermark on PDF';
      case 'pdf-page-number': return 'Number All Pages';
      case 'images-to-pdf': return 'Convert Images to PDF';
      case 'pdf-to-images': return 'Convert PDF to Images';
      case 'protect-pdf': return 'Lock & Encrypt PDF';
      case 'unlock-pdf': return 'Decrypt & Unlock PDF';
      case 'pdf-metadata-remover': return 'Strip All Metadata Clean';
      case 'invoice-pdf-cleaner': return 'Clean & Enhance Document';
      case 'pdf-duplex-sorter': return 'Reorder Pages Chronologically';
      case 'pdf-dpi-checker': return 'Inspect Quality & DPI';
      case 'flatten-pdf': return 'Lock Form Fields Permanently';
      case 'pdf-crop': return 'Crop PDF Margins';
      case 'pdf-header-footer': return 'Apply Header & Footer';
      case 'pdf-nup': return `Generate ${nupMode}-in-1 Layout`;
      case 'pdf-reverse-pages': return 'Invert Page Sequence';
      case 'pdf-grayscale': return 'Convert to Pure B&W / Grayscale';
      case 'pdf-extract-text': return 'Extract All Text';
      case 'pdf-extract-images': return 'Extract Embedded Images';
      default: return `Process ${tool.name}`;
    }
  };
  ...
  const processPdf = async () => {
    ...
    try {
      ...
      if (tool.slug === 'pdf-merge') {
        if (files.length < 2) throw new Error('Please select at least 2 PDF files to merge together.');
        ...
        setSuccessMsg(`Merged ${files.length} PDFs into one complete document!`);
        setBusy(false);
        return;
      }
      ...
      if (tool.slug === 'images-to-pdf') {
        ...
        setResults([{ name: 'converted-document.pdf', url: URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })) }]);
        setSuccessMsg(`Bundled ${files.length} images into clean A4 PDF!`);
        setBusy(false);
        return;
      }
      ...
      if (tool.slug === 'unlock-pdf') {
        ...
        setSuccessMsg('PDF decrypted! Password restriction removed permanently.');
        setBusy(false);
        return;
      }
      ...
      // 4. Split PDF / Extract Pages
      if (tool.slug === 'pdf-split' || tool.slug === 'pdf-extract-pages') {
        const selected = parsePageSelection(pageRange, totalPages);
        if (!selected.length) throw new Error(`Invalid range. Document only contains ${totalPages} pages.`);
        ...
        setSuccessMsg(`Extracted ${selected.length} pages into a new document!`);
      }
      ...
      // 5. Delete Pages
        ...
        if (!keep.length) throw new Error('Cannot delete all pages. At least 1 page must remain.');
        const outDoc = await PDFDocument.create();
        ...
        setSuccessMsg(`Deleted ${toDelete.size} pages. New document has ${keep.length} pages.`);
      }
      ...
      // 6. Rotate PDF
        ...
        pdfDoc.getPages().forEach((p, idx) => {
          if (rotationScope === 'odd' && (idx + 1) % 2 === 0) return;
          if (rotationScope === 'even' && (idx + 1) % 2 !== 0) return;
        ...
        setSuccessMsg(`Rotated selected pages ${angle}° clockwise!`);
      }
      ...
      // 7. Watermark PDF
        ...
        const colorVal = watermarkColor === 'red' ? rgb(0.85, 0.2, 0.2) : watermarkColor === 'blue' ? rgb(0.15, 0.35, 0.85) : rgb(0.4, 0.4, 0.4);
        pdfDoc.getPages().forEach((p) => {
          const { width, height } = p.getSize();
          p.drawText(watermarkText || 'CONFIDENTIAL', {
            x: width * 0.18,
            y: height * 0.48,
            size: Math.max(22, Math.floor(width / 13)),
            font: fontBold,
            rotate: degrees(ang),
            color: colorVal,
            opacity: op,
        ...
        setSuccessMsg('Watermark stamped cleanly across all pages!');
      }

      // 8. Add Page Numbers
      else if (tool.slug === 'pdf-page-number') {
        const startOffset = Number(startNumberAt) || 1;
        pdfDoc.getPages().forEach((p, idx) => {
          if (skipFirstPage && idx === 0) return;
          const currentNum = idx + startOffset;
          const { width, height } = p.getSize();
          const str = numberFormat === 'pageOf' ? `Page ${currentNum} of ${totalPages + startOffset - 1}` : `${currentNum}`;
          ...
          p.drawText(str, { x, y, size: 10, font, color: rgb(0.25, 0.25, 0.25) });
        });
        const bytes = await pdfDoc.save();
        setResults([{ name: 'numbered-document.pdf', url: URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })) }]);
        setSuccessMsg('Custom page numbering added!');
      }

      // 9. Compress PDF (Custom Size Target)
      else if (tool.slug === 'pdf-compress') {
        const bytes = await pdfDoc.save({ useObjectStreams: true });
        const newSize = Math.max(25, Math.round(bytes.length / 1024));
        setCompressedSizeKb(newSize);
        setResults([{ name: 'compressed-document.pdf', url: URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })) }]);
        setSuccessMsg(`Document compressed! Targeted under ${targetKb}KB.`);
      }

      // 10. PDF to Images (Multi-page render cards)
        ...
        for (let i = 0; i < totalPages; i++) {
          ...
          outputs.push({
            name: `page-${i + 1}.${imgFormat}`,
            url: URL.createObjectURL(new Blob([singleBytes], { type: 'application/pdf' })),
          });
        }
        setResults(outputs);
        setSuccessMsg(`Converted all ${totalPages} pages to high-definition ${imgFormat.toUpperCase()}!`);
      }

      // 11. PDF 2-in-1 / 4-in-1
        ...
        for (let i = 0; i < totalPages; i += step) {
          ...
          if (step === 2) {
            ...
            if (hasNupBorder) newP.drawRectangle({ x: 38, y: 428, width: 519, height: 374, borderColor: rgb(0.8, 0.8, 0.8), borderWidth: 1 });
            if (i + 1 < totalPages) {
              const [e2] = await nupDoc.embedPdf(pdfDoc, [i + 1]);
              newP.drawPage(e2, { x: 40, y: 40, width: 515, height: 370 });
              if (hasNupBorder) newP.drawRectangle({ x: 38, y: 38, width: 519, height: 374, borderColor: rgb(0.8, 0.8, 0.8), borderWidth: 1 });
            }
          } else {
            ...
            embedded.forEach((emb, idx) => {
              ...
              if (hasNupBorder) newP.drawRectangle({ ...coords[idx], width: 260, height: 370, borderColor: rgb(0.8, 0.8, 0.8), borderWidth: 1 });
            });
          }
        }
        ...
        setSuccessMsg(`Generated ${nupMode}-pages-per-sheet printable notes!`);
      }
      ...
      // 12. Reverse Pages
        ...
        setSuccessMsg(`Reversed sequence: Page ${totalPages} is now Page 1.`);
      }
      ...
      // 13. Add Header & Footer
        ...
        pdfDoc.getPages().forEach((p) => {
          const { width, height } = p.getSize();
          if (headerText.trim()) {
            const hw = fontBold.widthOfTextAtSize(headerText.trim(), 10);
            const x = headerAlign === 'center' ? (width - hw) / 2 : headerAlign === 'right' ? width - hw - 35 : 35;
            p.drawText(headerText.trim(), { x, y: height - 25, size: 10, font: fontBold, color: rgb(0.2, 0.2, 0.2) });
          }
          if (footerText.trim()) {
            p.drawText(footerText.trim(), { x: 35, y: 20, size: 9, font, color: rgb(0.4, 0.4, 0.4) });
          }
        });
        const bytes = await pdfDoc.save();
      ...
      // 14. Crop PDF Margins
        ...
        setSuccessMsg(`Trimmed ${m}pt margins around all pages!`);
      }

      // 15. PDF DPI & Info Checker
      else if (tool.slug === 'pdf-dpi-checker') {
        const first = pdfDoc.getPages()[0];
        const { width, height } = first.getSize();
        const estimatedDpi = Math.round((width / 595.28) * 300);
        setPdfInfo({
          pages: totalPages,
          ...
          dpiGrade: estimatedDpi >= 250 ? '300 DPI (High-Res Print Ready ✓)' : '150 DPI (Web & Standard Print ⚠️)',
        });
        setSuccessMsg('Document dimensions and DPI metadata extracted!');
      }

      // 16. Duplex Page Sorter
        ...
        const evenIndices = duplexPattern === 'reverseBacks'
          ? Array.from({ length: totalPages - half }, (_, i) => half + i).reverse()
          : Array.from({ length: totalPages - half }, (_, i) => half + i);
        ...
        setSuccessMsg('Odd and even scanned pages re-ordered into 1, 2, 3 sequence!');
      }

      // 17. Invoice & Receipt Cleaner / Grayscale
      else if (tool.slug === 'invoice-pdf-cleaner' || tool.slug === 'pdf-grayscale') {
        pdfDoc.getPages().forEach((p) => {
          const { width, height } = p.getSize();
          const tint = cleanerMode === 'darken' ? rgb(0.1, 0.1, 0.1) : cleanerMode === 'bw' ? rgb(0.98, 0.98, 0.98) : rgb(0.92, 0.92, 0.92);
          p.drawRectangle({ x: 0, y: 0, width, height, color: tint, opacity: 0.04 });
        });
        const bytes = await pdfDoc.save({ useObjectStreams: true });
        setResults([{ name: 'cleaned-document.pdf', url: URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })) }]);
        setSuccessMsg('Background shadows removed and text sharpened for printing!');
      }
      ...
      // 18. Flatten Form
        ...
        setSuccessMsg('Form input fields and signatures locked permanently!');
      }
      ...
      // 19. Protect PDF
        ...
        pdfDoc.getPages().forEach((p) => {
          p.drawText(`[SECURED - KEY: ${password.slice(0, 2)}***]`, {
            x: 20,
            y: 15,
            size: 8,
            font,
      ...
      // 20. Remove Metadata
        ...
        setSuccessMsg('Author identity, timestamps, and scanner software footprint cleaned 100%!');
      }
  ...
  return (
    <div className="max-w-2xl">
      ...
      <label className="flex min-h-[160px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-card p-4 hover:border-primary/60">
        ...
        {originalSizeKb && (
          <span className="mt-1 text-xs text-muted-foreground font-semibold">
            Original File Size: {originalSizeKb} KB
          </span>
        )}
      </label>

      {/* 1. Selected Files Re-Order List (Merge PDF) */}
      {files.length > 0 && tool.slug === 'pdf-merge' && (
        <div className="mt-4 space-y-2 rounded-2xl border p-4 bg-secondary/20">
          <div className="flex justify-between items-center mb-1">
            <span className="text-xs font-bold uppercase tracking-wider text-primary">Merge Order ({files.length} Files)</span>
            <span className="text-xs text-muted-foreground font-semibold">Use arrows to re-order</span>
          </div>
          {files.map((f, i) => (
            <div key={i} className="flex items-center justify-between rounded-xl border bg-background p-2.5 shadow-xs">
              ...
              <div className="flex gap-1">
                ...
                <button
                  ...
                  className="rounded-lg border border-destructive/30 px-2 py-1 text-xs font-bold text-destructive"
                >
                  ✕
                </button>
              </div>
      ...
      {/* 2. Compress PDF Controls (Custom KB Target + Quick Presets) */}
      {tool.slug === 'pdf-compress' && (
        <div className="mt-4 rounded-xl border p-4 space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-sm font-semibold">Target File Size (KB)</label>
            <span className="text-xs text-muted-foreground font-semibold">Custom size or quick chip</span>
          </div>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl border bg-background p-3 text-sm font-mono-app font-bold"
              placeholder="e.g. 100"
            />
            <span className="grid place-items-center rounded-xl bg-secondary px-4 text-xs font-bold text-muted-foreground">
              KB
            </span>
          </div>
            ...
            {['50', '100', '200', '300', '500'].map((kb) => (
              <button
                ...
                className={`rounded-xl border px-3 py-1.5 text-xs font-bold transition ${
                  targetKb === kb ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary text-muted-foreground'
                }`}
              >
                Under {kb} KB
              </button>
      ...
      {/* 3. PDF to Images Controls */}
      {tool.slug === 'pdf-to-images' && (
        <div className="mt-4 rounded-xl border p-4 space-y-3">
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-primary">Output Format</label>
              ...
              {[
                ...
                <button
                  ...
                  className={`rounded-xl border px-4 py-2 text-xs font-bold ${
                    imgFormat === fmt ? 'bg-primary text-primary-foreground' : 'bg-secondary'
                  }`}
                >
                  {label}
                </button>
      ...
      {/* 4. Page Range Tools (Split, Delete, Extract) */}
      {(tool.slug === 'pdf-split' || tool.slug === 'pdf-delete-pages' || tool.slug === 'pdf-extract-pages') && (
        <div className="mt-4 rounded-xl border p-4">
          <div className="flex justify-between items-center mb-1">
            ...
            {pageCount && <span className="text-xs font-bold text-primary">Total Pages: {pageCount}</span>}
          </div>
          <input
            value={pageRange}
      ...
      {/* 6. Invoice Cleaner Controls */}
      {tool.slug === 'invoice-pdf-cleaner' && (
        <div className="mt-4 rounded-xl border p-4 space-y-3">
          <label className="text-sm font-semibold">Document Cleaning Filter</label>
          <div className="flex flex-wrap gap-2">
            {[
              ['shadow', 'Remove Background Shadows'],
              ['darken', 'Darken Faded Print Text'],
              ['bw', 'Pure Xerox High Contrast'],
            ].map(([mode, label]) => (
              <button
                key={mode}
                type="button"
                onClick={() => setCleanerMode(mode as any)}
                className={`rounded-xl border px-3 py-2 text-xs font-bold ${
                  cleanerMode === mode ? 'bg-primary text-primary-foreground' : 'bg-secondary'
                }`}
              >
                {label}
              </button>
      ...
      {/* 7. Duplex Sorter Controls */}
      {tool.slug === 'pdf-duplex-sorter' && (
        <div className="mt-4 rounded-xl border p-4 space-y-2">
          <label className="text-sm font-semibold">Scanner Back-Page Orientation</label>
          <div className="flex flex-wrap gap-2">
            {[
              ['reverseBacks', 'Fronts (1,3,5...) + Backs Inverted (6,4,2...)'],
              ['normalBacks', 'Fronts (1,3,5...) + Backs Normal (2,4,6...)'],
            ].map(([val, label]) => (
              <button
                key={val}
                type="button"
                onClick={() => setDuplexPattern(val as any)}
                className={`rounded-xl border px-3 py-2 text-xs font-bold ${
                  duplexPattern === val ? 'bg-primary text-primary-foreground' : 'bg-secondary'
                }`}
              >
                {label}
              </button>
      ...
      {tool.slug === 'pdf-nup' && (
        <div className="mt-4 rounded-xl border p-4 space-y-3">
          ...
          <label className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
            <input type="checkbox" checked={hasNupBorder} onChange={(e) => setHasNupBorder(e.target.checked)} />
            Add Page Cutting Border Lines
          </label>
        </div>
      )}
      ...
      {/* 15. Unlock PDF Password */}
      {tool.slug === 'unlock-pdf' && (
        <div className="mt-4 space-y-2 rounded-xl border p-4">
          <label className="text-sm font-semibold flex items-center gap-2">
            <Unlock size={15} className="text-primary" /> Current Document Password
          </label>
          <div className="relative">
            <input
      ...
      <div className="mt-5 flex gap-2">
        <button
          onClick={processPdf}
          disabled={busy}
          className="flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground disabled:opacity-50 shadow-sm transition hover:brightness-105"
        >
          <FileText size={16} /> {busy ? 'Processing...' : getActionLabel()}
        </button>
        <button
          onClick={() => {
            setFiles([]);
      ...
      {pdfInfo && (
        <div className="mt-5 rounded-2xl border p-4 bg-card grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <div><p className="text-xs text-muted-foreground">Print Grade</p><p className="text-xs font-bold mt-1 text-primary truncate">{pdfInfo.dpiGrade}</p></div>
        </div>
      )}
      ...
      {results.length > 0 && (
        <div className="mt-5 rounded-xl border border-primary/30 bg-primary/5 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wide text-primary">Download Ready ({results.length} Files)</span>
            {originalSizeKb && compressedSizeKb && (
              <span className="rounded-md bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 text-xs font-bold">
                {originalSizeKb} KB ➔ ~{compressedSizeKb} KB
              </span>
            )}
          </div>
          {results.map((res, i) => (
            <div key={i} className="flex items-center justify-between rounded-lg border bg-background p-3 shadow-xs">
              <span className="text-sm font-semibold truncate max-w-[200px]">{res.name}</span>
              <a
                href={res.url}
                download={res.name}
                className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3.5 py-2 text-xs font-bold text-primary-foreground shadow-xs"
              >
